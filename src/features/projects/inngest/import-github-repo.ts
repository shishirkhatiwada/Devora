...
export const importGithubRepo = inngest.createFunction(
  {
    ...
    onFailure: async ({ event, step }) => {
      ...
      await step.run("set-failed-status", async () => {
        await convex.mutation(api.systm.updateImportStatus, {
          internalKey,
          projectId,
          status: "failed",
        });
  ...
  async ({ event, step }) => {
    ...
    await step.run("cleanup-project", async () => {
      await convex.mutation(api.systm.cleanup, { 
        internalKey,
        projectId
      });
    });
    ...
    const folderIdMap = await step.run("create-folders", async () => {
      ...
      for (const folder of folders) {
        ...
        const folderId = await convex.mutation(api.systm.createFolder, {
          internalKey,
          projectId,
          name,
          parentId,
      ...
      for (const file of allFiles) {
        ...
        try {
          ...
          if (isBinary) {
            ...
            await convex.mutation(api.systm.createBinaryFile, {
              internalKey,
              projectId,
              name,
              storageId,
            ...
            await convex.mutation(api.systm.createFile, {
              internalKey,
              projectId,
              name,
              content,
