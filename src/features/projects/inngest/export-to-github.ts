...
export const exportToGithub = inngest.createFunction(
  {
    ...
    onFailure: async ({ event, step }) => {
      ...
      await step.run("set-failed-status", async () => {
        await convex.mutation(api.systm.updateExportStatus, {
          internalKey,
          projectId,
          status: "failed",
        });
  ...
  async ({ event, step }) => {
    ...
    const { data: repo } = await step.run("create-repo", async () => {
      return await octokit.rest.repos.createForAuthenticatedUser({
        name: repoName,
        description: description || `Exported from Devora`,
    ...
    const files = await step.run("fetch-project-files", async () => {
      return (await convex.query(api.systm.getProjectFilesWithUrls, {
        internalKey,
        projectId,
      })) as FileWithUrl[];
    });
    ...
    const { data: commit } = await step.run("create-commit", async () => {
      return await octokit.rest.git.createCommit({
        owner: user.login,
        repo: repoName,
        message: "Initial commit from Devora",
        tree: tree.sha,
